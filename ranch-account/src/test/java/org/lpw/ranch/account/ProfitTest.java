...
public class ProfitTest extends TestSupport {
    ...
    public void profit() {
        ...
        mockHelper.mock("/account/profit");
        ...
        for (String property : new String[]{"type", "deposit", "withdraw", "reward", "consume", "pending"})
            Assert.assertEquals(0, data.getIntValue(property));
        for (String property : new String[]{"balance", "profit"})
        ...
        Assert.assertEquals(1, account.getProfit());
        Assert.assertEquals(0, account.getConsume());
        Assert.assertEquals(0, account.getPending());
        Assert.assertEquals(digest.md5(AccountModel.NAME + ".service.checksum&user 1&owner 1&0&1&0&0&0&1&0&0"), account.getChecksum());
        ...
        Assert.assertEquals(0, account.getWithdraw());
        Assert.assertEquals(0, account.getReward());
        Assert.assertEquals(2, account.getProfit());
        Assert.assertEquals(0, account.getConsume());
        Assert.assertEquals(0, account.getPending());
        Assert.assertEquals(digest.md5(AccountModel.NAME + ".service.checksum&sign in id&owner 2&0&2&0&0&0&2&0&0"), account.getChecksum());
        ...
        Assert.assertTrue(accountTypes.get(AccountTypes.PROFIT).complete(null, null));
    }
}
