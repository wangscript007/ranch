...
public class PassTesterImpl implements PassTester {
    @Autowired
    ...
    @Override
    public <T extends AuditModel> void pass(AuditTesterDao<T> testerDao, String name, int code) {
        ...
        Assert.assertEquals("", object.getString("data"));
        for (int i = 0; i < 3; i++)
            Assert.assertEquals(Audit.Passed.getValue(), testerDao.findById(list.get(i).getId()).getAudit());
