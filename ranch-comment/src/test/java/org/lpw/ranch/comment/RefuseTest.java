 ...
 * @author lpw
 */
public class RefuseTest extends TestSupport {
    @Test
    public void refuse() {
        clean();
        ...
        mockHelper.mock("/comment/refuse");
        ...
        Assert.assertEquals("", object.getString("data"));
        for (int i = 0; i < 3; i++)
            Assert.assertEquals(Audit.Refused.getValue(), findById(list.get(i).getId()).getAudit());
